...
export default function Blog() {
  ...
  useEffect(() => {
    document.title = "Blog — Moukis tech";
  ...
  return (
    <div className="bg-[#050507] text-white min-h-screen overflow-x-hidden">
      ...
      <main className="mx-auto max-w-7xl px-5 sm:px-8 pt-36 pb-28">
        <Overline>{t.blog.overline}</Overline>
        <h1 className="mt-4 font-display font-900 text-5xl sm:text-7xl tracking-tightest leading-[0.95] max-w-3xl">{t.blog.title}</h1>
        <p className="mt-6 font-body text-white/50 max-w-xl">{t.blog.desc}</p>

        {loading ? (
          <p className="mt-16 font-mono-tech text-white/40">{t.blog.loading}</p>
        ) : posts.length === 0 ? (
          <p className="mt-16 font-mono-tech text-white/40">{t.blog.empty}</p>
        ) : (
                ...
                <Link to={`/blog/${p.slug}`} className="group block rounded-2xl border border-white/10 bg-white/[0.03] overflow-hidden hover:border-[#00E5FF]/40 transition-colors">
                  ...
                  <div className="p-6">
                    <div className="flex gap-2 flex-wrap">
                      {p.tags?.slice(0, 2).map((tag) => <span key={tag} className="font-mono-tech text-[10px] uppercase tracking-wider text-[#00E5FF]">#{tag}</span>)}
                    </div>
                    ...
                    <span className="mt-4 inline-flex items-center gap-1 font-mono-tech text-xs text-white/60 group-hover:text-white">{t.blog.read} <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" /></span>
                  </div>
                </Link>
              </motion.div>
            ))}
