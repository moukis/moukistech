...
const LINE1 = ["Réparation", "d'ordinateurs"];
const LINE2 = ["portables", "de", "confiance."];
    ...
    <section ref={ref} id="accueil" data-testid="hero-section" className="relative min-h-screen overflow-hidden flex items-center">
      ...
      <motion.div style={{ y: contentY, opacity: contentOpacity }} className="relative z-10 mx-auto max-w-7xl px-5 sm:px-8 w-full pt-28">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
          <Overline>Technicien Laptop &amp; Électronique · {CONTACT.country}</Overline>
        </motion.div>
        ...
        >
          ...
          <span className="block overflow-hidden">
            {LINE2.map((w, i) => (
              <span key={i} className="inline-block overflow-hidden mr-[0.25em]">
                <motion.span variants={word} className={`inline-block ${w === "confiance." ? "text-[#0055FF]" : ""}`}>{w}</motion.span>
              </span>
            ))}
          </span>
        </motion.h1>
        ...
        >
          Diagnostic rapide, réparations de qualité, et service honnête. Confiez votre matériel à un expert passionné.
        </motion.p>
...
export function Marquee() {
  const items = ["DIAGNOSTIC RAPIDE", "PIÈCES DE QUALITÉ", "SERVICE HONNÊTE", "TECHNICIEN EXPERT", "PRIX TRANSPARENTS"];
      ...
      <div className="flex w-max animate-marquee whitespace-nowrap">
        {loop.map((t, i) => (
          <span key={i} className="mx-8 font-display font-800 text-4xl sm:text-6xl text-stroke uppercase tracking-tight flex items-center gap-8">
            {t} <span className="text-[#0055FF] not-italic" style={{ WebkitTextStroke: 0 }}>◆</span>
          </span>
        ))}
      </div>
    </div>
