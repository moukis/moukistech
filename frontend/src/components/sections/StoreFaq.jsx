...
import { STORE_ICONS } from "@/data/content";
        ...
        <div className="mx-auto max-w-7xl px-5 sm:px-8 relative">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-14">
            <div className="max-w-2xl">
              <Overline>{t.store.overline}</Overline>
              <h2 className="mt-4 font-display font-800 text-4xl sm:text-5xl lg:text-6xl tracking-tightest leading-[0.95]">{t.store.title}</h2>
              <p className="mt-6 font-body text-white/50">{t.store.desc}</p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
            {t.store.items.map((name, i) => {
              const Icon = STORE_ICONS[i];
              return (
                <motion.div
                  key={name}
                ...
                >
                  ...
                  <p className="font-body text-xs text-white/70 leading-snug">{name}</p>
                </motion.div>
              );
            })}
          </div>
      ...
      <section id="faq" data-testid="faq-section" className="relative py-28 sm:py-40">
        <div className="mx-auto max-w-4xl px-5 sm:px-8">
          <div className="max-w-2xl mb-12">
            ...
            <h2 className="mt-4 font-display font-800 text-4xl sm:text-5xl tracking-tightest leading-[0.95]">{t.faq.title}</h2>
          </div>
