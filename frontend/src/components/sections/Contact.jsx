...
export default function Contact() {
  ...
  const validate = () => {
    ...
    if (!EMAIL_RE.test(form.email)) e.email = "Adresse email invalide.";
  ...
  const submit = async (ev) => {
    ...
    if (!validate()) return;
    setLoading(true);
    const message = date ? `${form.message}\n\nDate souhaitée : ${format(date, "PPP", { locale: fr })}` : form.message;
    try {
      await api.post("/contact", { ...form, message });
      toast.success("Message envoyé ! Nous vous recontactons très vite.");
      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-8 relative grid gap-14 lg:grid-cols-12">
        <div className="lg:col-span-5">
          ...
          <h2 className="mt-4 font-display font-800 text-4xl sm:text-5xl tracking-tightest leading-[0.98]">
            Réservez votre réparation dès aujourd'hui.
          </h2>
          <p className="mt-6 font-body text-white/50">Un problème avec votre ordinateur ? Contactez-moi directement, je réponds rapidement.</p>
          ...
          <div className="mt-10 space-y-4">
            <a href={`tel:${CONTACT.phoneRaw}`} data-testid="contact-phone" className="flex items-center gap-4 rounded-xl border border-white/10 bg-white/[0.03] p-5 hover:border-[#00E5FF]/40 transition-colors">
              ...
              <span><span className="block font-mono-tech text-xs text-white/40 uppercase">Téléphone</span><span className="font-600">{CONTACT.phone}</span></span>
            </a>
            <a href={`mailto:${CONTACT.email}`} data-testid="contact-email" className="flex items-center gap-4 rounded-xl border border-white/10 bg-white/[0.03] p-5 hover:border-[#00E5FF]/40 transition-colors">
              ...
              <span><span className="block font-mono-tech text-xs text-white/40 uppercase">Email</span><span className="font-600">{CONTACT.email}</span></span>
            </a>
            <a href={`https://wa.me/${CONTACT.whatsapp}`} target="_blank" rel="noopener noreferrer" data-testid="contact-whatsapp" className="flex items-center gap-4 rounded-xl border border-white/10 bg-white/[0.03] p-5 hover:border-[#25D366]/40 transition-colors">
              ...
              <span><span className="block font-mono-tech text-xs text-white/40 uppercase">WhatsApp</span><span className="font-600">Discuter maintenant</span></span>
            </a>
            <div className="flex items-center gap-4 rounded-xl border border-white/10 bg-white/[0.03] p-5">
              ...
              <span><span className="block font-mono-tech text-xs text-white/40 uppercase">Zone</span><span className="font-600">{CONTACT.country} — Déplacements & envoi</span></span>
            </div>
          </div>

          <div className="mt-6 overflow-hidden rounded-xl border border-white/10" data-testid="contact-map">
            <iframe title="Carte" src="https://www.openstreetmap.org/export/embed.html?bbox=2.224%2C48.815%2C2.469%2C48.902&layer=mapnik" className="w-full h-56 grayscale invert-[0.9] contrast-125" loading="lazy" />
          </div>
        </div>
        ...
        <div className="lg:col-span-7">
          ...
          >
            <div className="grid gap-5 sm:grid-cols-2">
              <div>
                <label className="font-mono-tech text-xs uppercase tracking-wider text-white/50">Nom complet</label>
                <input data-testid="contact-name-input" value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="Jean Dupont" className={`mt-2 ${inputCls}`} />
                {errors.name && <p data-testid="error-name" className="mt-1 text-xs text-red-400">{errors.name}</p>}
              </div>
              <div>
                <label className="font-mono-tech text-xs uppercase tracking-wider text-white/50">Email</label>
                <input data-testid="contact-email-input" value={form.email} onChange={(e) => set("email", e.target.value)} placeholder="jean@email.com" className={`mt-2 ${inputCls}`} />
                {errors.email && <p data-testid="error-email" className="mt-1 text-xs text-red-400">{errors.email}</p>}
              </div>
              <div>
                <label className="font-mono-tech text-xs uppercase tracking-wider text-white/50">Téléphone</label>
                <input data-testid="contact-phone-input" value={form.phone} onChange={(e) => set("phone", e.target.value)} placeholder="+33 ..." className={`mt-2 ${inputCls}`} />
              </div>
              <div>
                <label className="font-mono-tech text-xs uppercase tracking-wider text-white/50">Service souhaité</label>
                <Select value={form.service} onValueChange={(v) => set("service", v)}>
                  ...
                  <SelectContent className="bg-[#0E0E12] border-white/10 text-white max-h-64">
                    {SERVICES.map((s) => <SelectItem key={s.title} value={s.title} className="focus:bg-[#0055FF] focus:text-white">{s.title}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            </div>
            ...
            <div className="mt-5">
              <label className="font-mono-tech text-xs uppercase tracking-wider text-white/50">Date souhaitée</label>
              <Popover>
                <PopoverTrigger asChild>
                  <button type="button" data-testid="contact-date-btn" className={`mt-2 flex items-center gap-3 ${inputCls} text-left`}>
                    ...
                    {date ? format(date, "PPP", { locale: fr }) : <span className="text-white/30">Sélectionner une date (optionnel)</span>}
                  </button>
                </PopoverTrigger>
            ...
            <div className="mt-5">
              <label className="font-mono-tech text-xs uppercase tracking-wider text-white/50">Décrivez le problème</label>
              <textarea data-testid="contact-message-input" value={form.message} onChange={(e) => set("message", e.target.value)} rows={5} placeholder="Mon ordinateur ne s'allume plus..." className={`mt-2 ${inputCls} resize-none`} />
            ...
            <button type="submit" data-testid="contact-submit-btn" disabled={loading}
              ...
              {loading ? "Envoi..." : <>Envoyer la demande <Send size={18} /></>}
            </button>
          </motion.form>
        </div>
      </div>
